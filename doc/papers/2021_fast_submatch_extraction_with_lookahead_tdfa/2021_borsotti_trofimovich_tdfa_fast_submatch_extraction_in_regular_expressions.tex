...
%To make it submatch-aware, one only needs to add $\epsilon$-transitions that correspond to submatch positions in RE,
%and modify the usual NFA simulation to save input position along each path that goes through these transitions.
...
as it is useful in practice %(e.g. for the  POSIX \texttt{regexec} function).
